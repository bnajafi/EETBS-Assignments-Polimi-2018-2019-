...
Folder_whereThoseTablesAre = r"/Users/apple/Desktop/TablesRLF"
...
def Ed_finder(row):
    ...
    DF_DiffuseIrradiance = pd.read_csv(path_file_Ed, sep = ";", index_col = 0, header = 0)
    ...
    Ed_Value = sp.interp(latitude,name_of_columns_as_numbers,DF_DiffuseIrradiance.loc[direction])
    return Ed_Value                
...
def ED_finder(row):
    ...
    path_file_ED = os.path.join(Folder_whereThoseTablesAre,name_file_ED)
    DF_BeamIrradiance = pd.read_csv(path_file_ED, sep = ";", index_col = 0, header = 0)
    name_of_columns = DF_BeamIrradiance.columns.get_values()
    name_of_columns_as_numbers = name_of_columns.astype(np.int32, copy=False)
    ED_Value = sp.interp(latitude,name_of_columns_as_numbers,DF_BeamIrradiance.loc[direction])    
    return ED_Value         
